...
definition(
    name: "Python Devices",
    namespace: "st_py_device",
    author: "Eric J McDonald",
    description: "Example SmartApp for Custom Python Devices",
...
preferences {
    page(name:"selectPyDevice", title:"Configuration", content:"selectPyDevice")
}
...
def selectPyDevice() {
    ...
    sendHubCommand(new physicalgraph.device.HubAction("lan discovery urn:schemas-upnp-org:device:PyDevice:", physicalgraph.device.Protocol.LAN))

    def devicesForDialog = getDevicesForDialog()

    // Only one page - can install or uninstall from this page
    return dynamicPage(name:"selectPyDevice", title:"", nextPage:"", refreshInterval: refreshInterval, install:true, uninstall: true) {
        section("") {
            input "selectedPyDevice", "enum", required:false, title:"Select Python Device \n(${devicesForDialog.size() ?: 0} found)", multiple:true, options:devicesForDialog
        }
    }
}
...
def locationHandler(evt) {
    ...
    log.debug('Py Device Received Response: ' + evt.description)
...
def initialize() {
    ...
    selectedPyDevice.each { ssdpUSN ->
        ...
        if (!d) {
            ...
            d = addChildDevice("py_device", "Python Device", dni, devices[ssdpUSN].hub, [
